<div class="p-6">
                ...
                <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                    <table class="w-full divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <th class="px-6 py-3 br-gray-50 sm:text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Route</th>
                        ...
                        @if($data->count())
                            @foreach($data as $item)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-left text-sm whitespace-no-wrap">{{$item->route}}</td>
                        ...
                        @else
                            <tr>
                                <td class="px-6 py-4 text-left text-sm whitespace-no-wrap" colspan="4">No Permissions Found</td>
                            </tr>
                        @endif
    ...
    <x-jet-dialog-modal wire:model="modalConfirmDeleteVisible">
        <x-slot name="title">
            {{ __('Delete the') }} {{('"')}} {{$role}} {{('-')}} {{$route}} {{('"')}} {{(' permission (')}}  {{$modelId}} {{(')')}}
        </x-slot>

        <x-slot name="content">
            {{ __('Are you sure you want to delete this permission? Once your permission has been deleted, all of its resources and data will be permanently deleted.') }}
